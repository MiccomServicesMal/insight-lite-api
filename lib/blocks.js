...
BlockController.prototype.checkBlockHash = function(req, res, next) {
  var self = this;
  var blockHashes = req.params.blockHash.split(',');
  var isValid = blockHashes.every(function(hash) {
    return hash.length >= 64 && isHexadecimal(hash);
  });
  if (!isValid) {
...
BlockController.prototype.block = function(req, res, next) {
  var self = this;
  async.map(req.blockHashes, function(blockHash, callback) {
    self._getBlock(blockHash, callback);
  }, function(err, blocks) {
...
BlockController.prototype._getBlock = function(hash, callback) {
  var self = this;
...
BlockController.prototype.transformBlock = function(block, info) {
  ...
  var transactions = blockObj.transactions.map(function(txObj) {
    return {
      id: txObj.hash,
      raw: bitcore.Transaction(txObj).toBuffer().toString('hex')
    };
  });
  return {
    hash: block.hash,
