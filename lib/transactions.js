...
TxController.prototype.transaction = function(req, res, next) {
  ...
  this.node.getDetailedTransaction(txid, function(err, transaction) {
    ...
    self.transformTransaction(transaction, transformOptions, function(err, transformedTransaction) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
...
TxController.prototype._getTransformOptions = function(req) {
  return {
    ...
    noRaw: parseInt(req.query.noRaw) ? true : false
  };
};
...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if (!options.noRaw) {
    transformed.rawtx = transaction.hex;
  }
...
TxController.prototype.multiRawTransaction = function(req, res, next, txids) {
  ...
  async.map(txids.split(','), function(txid, callback) {
    self.node.getTransaction(txid, callback);
  }, function(err, txs) {
    if (err && err.code === -5) {
      return self.common.handleErrors(null, res);
    } else if(err) {
    ...
    req.rawTransaction = txs.map(function(tx) {
      return {
